...
fn node_with_fields(
    ...
    writeln!(
        w,
        "{}{}(as(x, {}{})) :- {}node(x, _, \"{}\", _, _, _, _, _, _, _, _, _, _, _).",
...
fn declare_node(config: &PrivGenConfig, w: &mut impl Write) -> Result<(), GenError> {
    writeln!(w, ".type {}FileId <: symbol", config.type_prefix)?;
    writeln!(w, ".type {}FilePath = {}FileId", config.type_prefix, config.type_prefix)?;
    ...
    writeln!(
        ...
        vec![
            ...
            format!("file_id: {}FileId", config.type_prefix),
    ...
    writeln!(
        w,
        "{}node_text(x, y) :- {}node(x, _, _, _, _, _, _, _, _, _, _, _, _, y).",
    ...
    writeln!(
        w,
        ".decl {}file_path(id: {}FileId, path: {}FilePath)",
        config.relation_prefix, config.type_prefix, config.type_prefix
    )?;
    // This assumes the file_id *is* the file path.
    // If you create a separate file.csv, you would .input that instead.
    writeln!(
        w,
        "{}file_path(id, id) :- {}node(_, id, _, _, _, _, _, _, _, _, _, _, _, _).",
        config.relation_prefix, config.relation_prefix
    )?;

    writeln!(
        w,
        ".decl {}node_location(id: {}Node, file_id: {}FileId, offset: {}StartByte, line: {}StartRow, col: {}StartCol)",
        config.relation_prefix, config.type_prefix, config.type_prefix, config.type_prefix, config.type_prefix, config.type_prefix
    )?;
    // This rule pulls location info from the node relation
    writeln!(
        w,
        "{}node_location(id, file_id, start_byte, start_row, start_col) :- {}node(id, file_id, _, _, _, _, _, start_byte, _, start_row, start_col, _, _, _).",
        config.relation_prefix, config.relation_prefix
    )?;
